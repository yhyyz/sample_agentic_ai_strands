...
class Mem0ServiceClient:
    ...
    PG_CONFIG_BEDROCK = {
        ...
        "llm": {
            "provider": "aws_bedrock",
            "config": {
                "model": os.environ.get("LLM_MODEL","anthropic.claude-3-5-haiku-20241022-v1:0"),
                "temperature": 0.1,
                "max_tokens": 2000,
            },
        },
    ...
    def _initialize_client(self, config: Optional[Dict] = None) -> Any:
        ...
        if os.environ.get("MEM0_API_KEY"):
            logger.info("Using Mem0 Platform backend (MemoryClient)")
            return MemoryClient()
        ...
        if os.environ.get("OPENSEARCH_HOST"):
            logger.info("Using OpenSearch backend (Mem0Memory with OpenSearch)")
            return self._initialize_opensearch_client(config)

        logger.info("Using FAISS backend (Mem0Memory with FAISS)")
    ...
    def _initialize_postgresql_client(self, config: Optional[Dict] = None) -> Mem0Memory:
        ...
        pg_config = self.PG_CONFIG_BEDROCK if os.environ.get('STRANDS_MODEL_PROVIDER') == "bedrock" else self.PG_CONFIG_OPENAI
    ...
    try:
        ...
        if action == "store":
            ...
            if results_list:
                ...
                console.print(panel)
            logger.info(f"results_list:{results_list}")
            return ToolResult(
