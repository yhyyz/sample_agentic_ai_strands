...
async function retrieveContext(
  ...
  try {
    if (!process.env.knowledgeBaseId) {
      console.error("knowledgeBaseId is not provided");
      return {
        context: "Error: knowledgeBaseId is not provided",
        isRagWorking: false,
        ragSources: [],
      };
    }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "retrieve_from_aws_kb") {
    const { query, n = 3 } = args as Record<string, any>;
    try {
      const result = await retrieveContext(query, n);
